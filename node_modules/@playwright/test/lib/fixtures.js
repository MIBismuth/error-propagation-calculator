...
class FixturePool {
      ...
      for (const entry of Object.entries(fixtures)) {
        ...
        if (previous && options) {
          if (previous.scope !== options.scope) {
            (0, _globals.addFatalError)(`Fixture "${name}" has already been registered as a { scope: '${previous.scope}' } fixture defined in ${(0, _util.formatLocation)(previous.location)}.`, location);
            continue;
          }
          if (previous.auto !== options.auto) {
            (0, _globals.addFatalError)(`Fixture "${name}" has already been registered as a { auto: '${previous.scope}' } fixture defined in ${(0, _util.formatLocation)(previous.location)}.`, location);
            continue;
          }
        } else if (previous) {
          options = {
        ...
        if (!kScopeOrder.includes(options.scope)) {
          (0, _globals.addFatalError)(`Fixture "${name}" has unknown { scope: '${options.scope}' }.`, location);
          continue;
        }
        if (options.scope === 'worker' && disallowWorkerFixtures) {
          (0, _globals.addFatalError)(`Cannot use({ ${name} }) in a describe group, because it forces a new worker.\nMake it top-level in the test file or put in the configuration file.`, location);
          continue;
        }
        ...
        if (fn === undefined && options.option && previous) {
          ...
          while (!original.fromConfig && original.super) original = original.super;
          fn = original.fn;
        }
  ...
  validate() {
    ...
    const visit = registration => {
      ...
      for (const name of registration.deps) {
        ...
        if (!dep) {
          if (name === registration.name) (0, _globals.addFatalError)(`Fixture "${registration.name}" references itself, but does not have a base implementation.`, registration.location);else (0, _globals.addFatalError)(`Fixture "${registration.name}" has unknown parameter "${name}".`, registration.location);
          continue;
        }
        if (kScopeOrder.indexOf(registration.scope) > kScopeOrder.indexOf(dep.scope)) {
          (0, _globals.addFatalError)(`${registration.scope} fixture "${registration.name}" cannot depend on a ${dep.scope} fixture "${name}" defined in ${(0, _util.formatLocation)(dep.location)}.`, registration.location);
          continue;
        }
        if (!markers.has(dep)) {
          visit(dep);
          ...
          (0, _globals.addFatalError)(`Fixtures ${names.join(' -> ')} -> "${dep.name}" form a dependency cycle: ${regs.map(r => (0, _util.formatLocation)(r.location)).join(' -> ')}`, dep.location);
          continue;
        }
      }
  ...
  validateFunction(fn, prefix, location) {
    for (const name of fixtureParameterNames(fn, location)) {
      const registration = this.registrations.get(name);
      if (!registration) (0, _globals.addFatalError)(`${prefix} has unknown parameter "${name}".`, location);
    }
  }
...
function innerFixtureParameterNames(fn, location) {
  ...
  if (firstParam[0] !== '{' || firstParam[firstParam.length - 1] !== '}') (0, _globals.addFatalError)('First argument must use the object destructuring pattern: ' + firstParam, location);
