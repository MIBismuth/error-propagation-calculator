...
exports.create = exports.WorkerRunner = void 0;
...
class WorkerRunner extends _process.ProcessRunner {
  ...
  async _loadIfNeeded() {
    ...
    this._configLoader = await _configLoader.ConfigLoader.deserialize(this._params.config);
    this._testLoader = new _testLoader.TestLoader(this._configLoader.fullConfig());
  ...
  async _runTest(test, retry, nextTest) {
    const testInfo = new _testInfo.TestInfoImpl(this._configLoader.fullConfig(), this._project, this._params, test, retry, stepBeginPayload => this.dispatchEvent('stepBegin', stepBeginPayload), stepEndPayload => this.dispatchEvent('stepEnd', stepEndPayload));
    ...
    this.dispatchEvent('testEnd', buildTestEndPayload(testInfo));
    const preserveOutput = this._configLoader.fullConfig().preserveOutput === 'always' || this._configLoader.fullConfig().preserveOutput === 'failures-only' && testInfo._isFailure();
...
const create = params => new WorkerRunner(params);
