...
class TestTypeImpl {
  ...
  _currentSuite(location, title) {
    ...
    if (!suite) {
      (0, _globals.addFatalError)([`Playwright Test did not expect ${title} to be called here.`, `Most common reasons include:`, `- You are calling ${title} in a configuration file.`, `- You are calling ${title} in a file that is imported by the configuration file.`, `- You have two different versions of @playwright/test. This usually happens`, `  when one of the dependencies in your package.json depends on @playwright/test.`].join('\n'), location);
      return;
    }
    return suite;
  }
  ...
  _describe(type, location, title, fn) {
    ...
    for (let parent = suite; parent; parent = parent.parent) {
      if (parent._parallelMode === 'serial' && child._parallelMode === 'parallel') (0, _globals.addFatalError)('describe.parallel cannot be nested inside describe.serial', location);
    }
  ...
  _configure(location, options) {
    ...
    if (options.mode !== undefined) {
      if (suite._parallelMode !== 'default') (0, _globals.addFatalError)('Parallel mode is already assigned for the enclosing scope.', location);
      suite._parallelMode = options.mode;
      for (let parent = suite.parent; parent; parent = parent.parent) {
        if (parent._parallelMode === 'serial' && suite._parallelMode === 'parallel') (0, _globals.addFatalError)('describe.parallel cannot be nested inside describe.serial', location);
      }
    }
  }
  _modifier(type, location, ...modifierArgs) {
    ...
    if (!testInfo) {
      (0, _globals.addFatalError)(`test.${type}() can only be called inside test, describe block or fixture`, location);
      return;
    }
    if (typeof modifierArgs[0] === 'function') (0, _globals.addFatalError)(`test.${type}() with a function can only be called inside describe block`, location);
  ...
  _setTimeout(location, timeout) {
    ...
    const testInfo = (0, _globals.currentTestInfo)();
    if (!testInfo) {
      (0, _globals.addFatalError)(`test.setTimeout() can only be called from a test`, location);
      return;
    }
  ...
  async _step(location, title, body) {
    const testInfo = (0, _globals.currentTestInfo)();
    if (!testInfo) {
      (0, _globals.addFatalError)(`test.step() can only be called from a test`, location);
      return undefined;
    }
