...
function addTestCommand(program) {
  ...
  command.option('--reporter <reporter>', `Reporter to use, comma-separated, can be ${_runner.builtInReporters.map(name => `"${name}"`).join(', ')} (default: "${_configLoader.baseFullConfig.reporter[0]}")`);
