...
{#if expanded}
	<div id="expanded-content" transition:slide on:click={(e) => e.stopPropagation()} on:keydown={handleKeyDown}>
		<nav>
			<h5 style="text-align: center; font-size: 2.5em;">Table of Contents</h5>
			<ul style="list-style-type: disc; margin-left: 30px;">
				<li><a href="#checking-your-expression">Entering your expression</a></li>
				<li><a href="#getting-your-expression">Getting your Expression (LaTex)</a></li>
				<li><a href="#Setting-your-variables">Setting your variables</a></li>
				<li><a href="#Error-Propagation-Output">Error Propagation Output</a></li>
			</ul>
		</nav>

		<h1 style="text-align: center; font-size: 2em;" id="checking-your-expression">
			Entering your expression
		</h1>
		<p class="flex flex-initial " style="margin:10px; pointer-events: none;">
			In order to start, you must enter your expression into the input box, like in the following
			example:
		</p>
		...
		<h2 style="text-align: center; font-size: 2em;" id="getting-your-expression">
			Getting your Expression (LaTex)
		</h2>
		<p style="margin:10px;">
			After entering your expression, in order to check whether it's the one you intended, you can
			click the "Get Expression" button, that converts the expression to LaTex and displays it.
			Clicking on the upper right corner's clipboard button, the LaTex code gets copied to your
			clipboard:
		...
		<p style="margin:10px; pointer-events: none;">
			In order to get the Error Propagation, you must first click the "Get Variables" button, which
			displays the following menu:
			<VariableMenu bind:VariableList />
			In this menu, every variable detected from your original expression gets its box, with the following
			attributes:
		</p>
		<ul style="list-style-type: circle !important; color:black; margin:30px;">
			<li>
				"Alias", which is an (optional) different name for the variable, which will show up in the
				Error Propagation expressions, and particularly useful for specifying the Excel cell where
				that variable is present;
			</li>
			<li>
				"Error", which has δ(variablename) as its default value. You can leave it in its symbolic
				representation or set the error, for example, to its corresponding Excel cell. Note that,
				like in the example shown for variable "y", in order to specifiy a fixed Excell cell, you
				must use the syntax "column_letter$row_number";
			</li>
			<li>
				Toggleable "Usable" setting, which determines whether the identified element is in fact a
				variable (and therefore suscetiple to an uncertainty and its consequent propagation), or
				just a constant. In this example, "a" is a constant, so this attribute is set on "Unusable".
			</li>
		</ul>

		<h4 style="text-align: center; font-size: 2em;" id="Error-Propagation-Output">
			Error Propagation Output
		</h4>
		<p style="margin:10px;">
			Finally, you can select how you want your ErrorPropagation, and for that, you have 2
			selectors:
		</p>
		<div class="flex m-3 gap-2 ">
			...
			<button
				...
				hover:bg-opacity-50 hover:rounded hover:m-0.5 transition-all">Get Error Propagation</button
			>
		</div>
		<p style="margin:10px;" />
		<p style="margin:10px;">
			The first one selects the output format of your ErrorPropagation equation, which is set by
			default to "Python". This way, the equation will be converted to Python code that you can
			easily copy to your own project using the clipboard button, in the upper right corner. You
			also have the option to convert the ErrorPropagation expression to an Excel formula (that,
			again, you can easily copy to Excel and then use its functionalities to apply it to the cells
			of your choosing). Finally, you can display the expression in LaTex.
		</p>

		<p style="margin:10px;">
			The second one selects the type of Propagation of Uncertainty: "Absolute" or "Quadratic" (see
			the section at the bottom of the webpage for their general expressions). Note that all
			expressions are already simplified.
		</p>
		<p style="margin:10px;">
			After setting the selectors to your preference, clicking "Get Error Propagation" will display
			your output in the "Expression Result" box, like in the following example (in this case, the
			chosen options were "Excel" and "Quadratic"):
		</p>
